package wonder.com.numbergame.view;
...
import wonder.com.numbergame.activity.Game;
import wonder.com.numbergame.bean.GameItem;
...
public class GameView extends GridLayout implements OnTouchListener {
    ...
    private void judgeDirection(int offsetX, int offsetY) {
        ...
        } else if (flagSuper && Config.mSuper) { // 启动超级用户权限来添加自定义数字
...
//        Log.d("JTest", String.valueOf(mTarget));
        for (int i = 0; i < mGameLines; i++) {
            for (int j = 0; j < mGameLines; j++) {
